...
class Header extends React.Component {
  ...
  changeCurrency = () => {
    this.state.displayCurrency
      ...
      : this.setState({ displayCurrency: true });
  };
  ...
  handleCartItem = () => {
    if (this.props.cartItems.length) {
      ...
      const totalCartQuanitity = cartQuanitity.reduce(
        (prev, next) => prev + next
      );
      ...
      <div>
        <div className="navbarContainer">
            ...
            <div>
              {
                <ul className="categoryList">
                  <li
                    onClick={() =>
                      this.handleAllSelection(
                        this.props.getCategories.categories[0].name
                      )
                    }
                    className={`navbarLink ${
                      (pathname === "/categories/all" ? true : false) &&
                      "navbarActiveLink"
                    }`}
                  >
                    All
                  ...
                  <li
                    onClick={() =>
                      this.handleClothesSelection(
                        this.props.getCategories.categories[1].name
                      )
                    }
                    className={`navbarLink ${
                      (pathname === "/categories/clothes" ? true : false) &&
                      "navbarActiveLink"
                    }`}
                  >
                    Clothes
                  ...
                  <li
                    onClick={() =>
                      this.handleTechSelection(
                        this.props.getCategories.categories[2].name
                      )
                    }
                    className={`navbarLink ${
                      (pathname === "/categories/tech" ? true : false) &&
                      "navbarActiveLink"
                    }`}
                  >
                    Tech
          ...
          <div className="rightSideContainer">
            <div onClick={this.changeCurrency} className="usdImageContainer">
              {currencies && (
                <div className="dollarSign">
                  {currencies[currencyState].symbol}
                </div>
              )}
              <div className="arrowImageContainer">
                <img
                  className={`faChevronDown ${
                    this.state.displayCurrency && "chevronDownActive"
                  }`}
            ...
            <div
              onClick={this.handledisplayMyBag}
              className="shoppingCartImageContainer"
            >
          ...
          {this.state.displayCurrency && (
            <Currency
              onOutsideClick={this.handleChangeCurrencyOutsideClick}
              className="currencyListContainer"
              onChangeCurrency={this.handleSelectCurrency}
...
export default withRouter(
  connect(mapStateToPtops)(
    compose(
      ...
      graphql(getCurrency, { name: "getCurrency" })
    )(Header)
  )
);
