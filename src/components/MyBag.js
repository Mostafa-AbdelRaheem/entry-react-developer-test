...
class MyBag extends React.Component {
      ...
      return (
        <div className="productContainer">
          {this.props.cartItems.map((cartItem, index) => (
            <CartProduct key={index} cartItemProps={cartItem} />
          ))}
        </div>
      );
    } else {
  ...
  handleTotalCost = () => {
    if (this.props.cartItems.length) {
      ...
      const costList = cartItems.map(
        (item) => item.prices[currencyState].amount * item.quantity
      );
      const total = costList.reduce((prev, next) => prev + next);
      return (
        <p className="totalCost">
          <span>{cartItems[0].prices[currencyState].currency.symbol}</span>
          {total.toFixed(2)}
        </p>
      );
    } else {
  ...
  render() {
    return (
      <div ref={this.wrapperRef} className="myBagContainer">
        <h3 className="myBagHeader">
          My Bag,{" "}
          <span className="myBagItemHeader">
            {this.props.cartItems.length} items
          </span>
        </h3>
        <div className="productsListContainer">{this.displayCart()}</div>
        ...
        <div className="buttonsContainer">
          <button onClick={this.handleToCartPage} className="viewBagBtn">
            VIEW BAG
          </button>
