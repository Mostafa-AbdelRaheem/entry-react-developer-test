...
class ProductDescription extends React.Component {
  ...
  hadnleGalleryDisplay = (picUrl) => {
    this.setState({ galleryPicUrl: picUrl });
  };
  ...
  handleAddToCart = (productProps, attributesSelected) => {
    const { id, name, description, prices, attributes, gallery, brand } =
      productProps;
      ...
      if (attributes.length === 0) {
        dispatch(
          addToCart({
            quantity: 1,
            id,
            productId: `${id}-${Object.values(attributesSelected).toString()}`,
            name,
            description,
            prices,
            attributes,
        ...
        <div className="productDecriptionContainer">
          ...
          <div className="galleryContainer">
            {gallery.map((pic, index) => (
              <div
                onClick={() => this.hadnleGalleryDisplay(pic)}
                key={index}
          ...
          <div className="info">
            ...
            <button
              disabled={!inStock}
              className={`${
                !inStock ? "addToCartBtn disabled" : "addToCartBtn"
              }`}
              onClick={() => {
  ...
  render() {
    ...
    return (
      <div className="ProductDescriptionContainer">{this.diplayProduct()}</div>
    );
  }
}
